...
import {
  ...
  prove,
  verify,
  proveWithdrawSignature,
  verifyWithdrawSignature,
} from "@ieigen/zkzru/operator/prover";
...
module.exports = function(app) {
  ...
  app.post("/zkzru/proveWithdraw", async (req, res) => {
    ...
    const fromAccount = await accountdb.findOne({
    ...
    const isValid1 = await verifyWithdrawSignature(
      withdrawProofResult["vk"],
      withdrawProofResult["proof"]
    );
    if (!isValid1) {
      throw new Error(`Error: the generated proof is not valid`);
    }
  ...
  app.post("/zkzru/depositSubTreeRoot", async (req, res) => {
    let result;
    try {
      result = await depositSubTreeRootdb.add(req.body.subTreeRoot);
    } catch (err) {
      console.log(err);
      throw err;
    }
